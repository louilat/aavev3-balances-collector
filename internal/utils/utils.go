...
func GetBlockReferences() map[int64]*big.Int {
	ref := map[int64]*big.Int{
		...
		time.Date(2025, 7, 1, 0, 0, 0, 0, time.FixedZone("UTC", 0)).Unix(): big.NewInt(23044301),
		time.Date(2025, 8, 1, 0, 0, 0, 0, time.FixedZone("UTC", 0)).Unix(): big.NewInt(23137364), // To change, month not finished yet
	}
	return ref
}
